...
import { ICON_MAP } from './guestbookIcons'
    ...
    return (
        <div className="bg-white rounded-xl p-4 shadow-sm">
            <div className="flex items-start gap-3">
                {/* 프로필 아이콘 */}
                <div className="flex items-center justify-center w-10 h-10 rounded-full bg-indigo-100 text-primary-dark">
                    {(() => { const Icon = ICON_MAP[item.icon] || Cat; return <Icon size={22} /> })()}
                </div>
                {/* 내용 */}
                    ...
                    {showDelete && (
                        <div className="flex gap-2 mt-2">
                            <input
                                ...
                                className="flex-1 px-3 py-1.5 border border-gray-300 rounded-lg text-xs focus:outline-none focus:border-primary"
                            />
                            <button
                                type="button"
                                onClick={handleDelete}
                                className="px-3 py-1.5 border border-primary text-primary rounded-lg text-xs hover:bg-gray-50 transition-colors"
                            >
                                삭제
                            </button>
                            <button
